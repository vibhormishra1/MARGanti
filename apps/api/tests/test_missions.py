...
async def test_mission_lifecycle(async_client: AsyncClient):
    ...
    create_res = await async_client.post("/api/v1/missions/", json=mission_payload)
    ...
    task_res = await async_client.post(f"/api/v1/missions/{mission_id}/tasks", json=task_payload)
    ...
    # 3. Publish Mission
    pub_res = await async_client.post(f"/api/v1/missions/{mission_id}/publish")
    assert pub_res.status_code == 200
    published = pub_res.json()
    assert published["status"] == "ACTIVE"

    # 4. Start Task
    start_res = await async_client.post(f"/api/v1/missions/{mission_id}/tasks/{task_id}/start")
    assert start_res.status_code == 200
    started_task = start_res.json()
    assert started_task["status"] == "IN_PROGRESS"

    # 5. Complete Task
    comp_task_res = await async_client.post(f"/api/v1/missions/{mission_id}/tasks/{task_id}/complete")
    assert comp_task_res.status_code == 200

    # 6. Complete Mission
    comp_mis_res = await async_client.post(f"/api/v1/missions/{mission_id}/complete")
