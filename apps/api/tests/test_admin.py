...
async def test_admin_update_user(async_client: AsyncClient):
    ...
    response = await async_client.patch("/api/v1/admin/users/unknown", json={"role": "commander"})
...
async def test_admin_update_config(async_client: AsyncClient):
    response = await async_client.put("/api/v1/admin/config/test_config", json={"value": {"feature_flag": True}})
    assert response.status_code == 200
    data = response.json()
    assert data["key"] == "test_config"
    assert data["value"]["feature_flag"] is True

    get_response = await async_client.get("/api/v1/admin/config/test_config")
...
async def test_admin_update_config_unauthorized(async_client: AsyncClient):
    ...
    response = await async_client.put(
        "/api/v1/admin/config/test_config?organization_id=other-org", json={"value": {"feature_flag": True}}
    )
