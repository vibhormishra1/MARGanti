...
class AdminService:
    ...
    async def update_config(
        self, key: str, organization_id: str, value: dict[str, Any], actor_id: str, actor_role: str
    ) -> SystemConfig:
        config = SystemConfig(
            key=key, organization_id=organization_id, value=value, updated_at=datetime.now(UTC), updated_by=actor_id
        )
    ...
    async def update_user(
        self, user_id: str, data: UserUpdate, actor_id: str, actor_role: str, actor_org_id: str
    ) -> UserResponse:
        user = await self.repo.update_user(user_id, data)
        if not user:
            raise ValueError("User not found")
        await self._audit(
            actor_id, actor_role, actor_org_id, "UPDATE", "User", user_id, "SUCCESS", data.model_dump(exclude_none=True)
        )
        return user
    ...
    async def update_organization(
        self, organization_id: str, data: OrganizationUpdate, actor_id: str, actor_role: str
    ) -> dict[str, Any]:
